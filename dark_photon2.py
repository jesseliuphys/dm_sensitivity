...
def main():
  ...
  plt.fill_between(haloscope['x'], haloscope['y'], 1e-1, edgecolor='none', facecolor=myDarkGray)

  dmsearch = pd.read_csv('limits/common/DP_Combined_DarkMatterSearches.txt', sep=' ', lineterminator='\n', names=['x', 'y'])
  plt.fill_between(dmsearch['x'], dmsearch['y'], 1e-1, edgecolor='none', facecolor=myDarkGray)  

  lab      = pd.read_csv('limits/common/DP_Combined_Laboratory.txt', sep=' ', lineterminator='\n', names=['x', 'y'])
  plt.plot(lab['x'], lab['y'], color=myMediumGray)
  plt.fill_between(lab['x'], lab['y'], 1e-1, edgecolor='none', facecolor=myMediumGray)
  
  dmcosmo  = pd.read_csv('limits/common/DP_Combined.txt', sep=' ', lineterminator='\n', names=['x', 'y'])
  plt.plot(dmcosmo['x'],   dmcosmo['y'],   color=myLightGray)
  plt.fill_between(dmcosmo['x'], dmcosmo['y'], 1e-1, edgecolor='none', facecolor=myLightGray)

  stellar  = pd.read_csv('limits/common/DP_Combined_Stellar.txt', sep='\t',lineterminator='\n', names=['x', 'y'])
  plt.fill_between(stellar['x'], stellar['y'], 1e-1, edgecolor='none', facecolor=myMediumGray)
  
  text_size = 23
  fig.text(0.18, 0.26,  r'Haloscopes',  color=myDarkGray, size=text_size)
  fig.text(0.75, 0.38,  r'Semiconductors',  color=myDarkGray, size=text_size)
  fig.text(0.22, 0.66,  r'Cosmology',  color=myMediumGray, size=text_size)
  fig.text(0.78, 0.67,  r'Stellar',  color=myLightGray, size=text_size)
  fig.text(0.23, 0.81,  r"$\gamma \to A'$",  color=myMediumGray, size=text_size)
  ...
  plt.plot(x, y, alpha=0.7,lw=3, ls='--', c=myMediumBlue, zorder=4) 
  plt.fill_between(x, y, 1e-1, edgecolor='none', facecolor=myMediumBlue, alpha=0.1)
  ...
  plt.fill_between(x, y, 1e-1, color=myDarkGray, alpha=0.3)
  ...
  plt.plot(x, y, alpha=0.8,lw=3, c=myMediumBlue, zorder=4) 
  plt.fill_between(x, y, 1e-1, edgecolor='none', facecolor=myMediumBlue, alpha=0.3)

  # KID [0.2, 5] meV
  x, y = calc_darkPhoton_coupling(3e-19, Adish, 0.2, 5, snr, effic, time)
  plt.plot(x, y, alpha=0.8,lw=3, c=myMediumOrange, zorder=4) 
  plt.fill_between(x, y, 1e-1, edgecolor='none', facecolor=myLightOrange, alpha=0.6)
  ...
  plt.fill_between(x, y, 1e-1, edgecolor='none', facecolor=myLightPink, alpha=0.6)
  ...
  plt.fill_between(x, y, 1e-1, color=myDarkGray, alpha=0.9)

  # Gentec [0.4, 120] meV
  x, y = calc_darkPhoton_coupling(1e-8, Adish, 0.4, 120, snr, effic, time)
  plt.plot(x, y, lw=3, ls='-', c=myDarkGreen) 
  plt.fill_between(x, y, 1e-1, edgecolor='none', facecolor=myMediumGreen, alpha=0.3)
  ...
  plt.plot(x, y, alpha=0.8,lw=1, c=myDarkBlue, zorder=4) 
  ...
  ax2 = ax.twiny()
  ax2.set_xlim(241.79e-6, 241.79)
  ax2.set_xscale('log')
  ax2.set_xlabel(r'$\nu~[\mathrm{THz}]$', labelpad=18, size=35)
  locmaj = mplt.ticker.LogLocator(base=10.0, subs=(1.0, ), numticks=50)
